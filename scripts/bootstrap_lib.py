...
def ensure_package(
    ...
    *,
    ...
    """Ensure package exists under workspace; clone if missing."""
    ...
    repo = meta.get("repo") or manifest["platform_repo"]
    branch = manifest.get("platform_branch", "main")
    subdir = meta["path"]
    dest.parent.mkdir(parents=True, exist_ok=True)

    if meta.get("repo"):
        # Dedicated repo — clone directly into dest
        print(f"→ Cloning {name} from {repo}")
        run(["git", "clone", "--branch", branch, "--depth", "1", repo, str(dest)])
        return dest

    # Monorepo sparse-checkout of a single package folder
    print(f"→ Fetching {name} from platform repo ({subdir})")
...
def write_orchestrator_yaml(workspace: Path, enabled_bots: list[str]) -> None:
    orch = workspace / "orchestrator"
    if not orch.exists():
        return
    bots_block = []
    catalog = {
        "instagram": ("Instagram", "../bot-instagram", 7411, "python -m ig_agent.api"),
        "linkedin": ("LinkedIn", "../bot-linkedin", 7412, "python -m linkedin_bot.api"),
        "x": ("X", "../bot-x", 7413, "python -m x_bot.api"),
    }
    for bot_id, (name, path, port, cmd) in catalog.items():
        enabled = bot_id in enabled_bots and (workspace / path.replace("../", "")).exists()
        bots_block.append(
            f"  - id: {bot_id}\n"
            f"    name: {name}\n"
            f"    path: {path}\n"
    ...
    print(f"✓ Wrote {orch / 'orchestrator.yaml'} (enabled: {enabled_bots})")
...
def bootstrap(
    *,
    ...
    bot_ids = [p.replace("bot-", "") for p in selected if p.startswith("bot-")]
    if "orchestrator" in selected:
        write_orchestrator_yaml(workspace, bot_ids)
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--profile", choices=["instagram", "fleet", "orchestrator-ig"], help="Preset package set")
